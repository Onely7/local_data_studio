...
- `src/local_data_studio/server/atlas.py` remains the compatibility facade for `src/local_data_studio/server/atlas_components`, which separates contracts, image conversion, projection, dataset caching, subprocess control, and orchestration. An encoder is created once per Atlas job and reused across anchor/transform batches. Transform row indexes are consumed in fixed-size batches, projection returns coordinates without materializing unused neighbor dictionaries, and concurrent misses for the same fingerprint/query/column/model/settings key share one cache generation. Cache pruning still removes old files first while preserving the parquet artifact needed by the current Atlas launch.
