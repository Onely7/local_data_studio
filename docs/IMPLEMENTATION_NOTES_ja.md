...
- アプリケーション本体は `src/local_data_studio` にあります。静的 UI ファイルは `src/local_data_studio/static` にあり、Python パッケージへ含まれます。実行時の `.env`、`data`、`cache`、`models/embedder` は、選択したワークスペースまたは現在の作業ディレクトリを基準にします。設定の優先順位は、コマンドラインオプション、OS の環境変数、設定ファイル、`.env`、ワークスペースのデフォルト、現在の作業ディレクトリのデフォルトの順です。
