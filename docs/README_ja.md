...
`[settings]` セクションには、`.env` を使わずに EDA、Embedding Atlas、元ファイルの削除許可を設定できます。環境変数名を小文字の snake_case へ置き換えて指定します。たとえば `EDA_ROW_LIMIT` は `eda_row_limit`、`ALLOW_DELETE_DATA` は `allow_delete_data` です。テンプレートには指定できる設定をすべて記載しています。キーを省略した場合は `.env` またはアプリケーションの既定値が使われます。
