...
7. **ブラウザーで画面を開く**
   ...
   リモートサーバーで起動する場合は、Local Data Studio のポートだけを転送します。

   ```bash
   ssh -N -L 8000:127.0.0.1:8000 user@example-server
   ```

   その後、同じローカル URL を開きます。Atlas ページは Local Data Studio 経由で配信されるため、Atlas の内部ポートを別途 SSH 転送する必要はありません。
...
- `ATLAS_HOST`、`ATLAS_PORT`: Embedding Atlas 子プロセスに使う IPv4 ループバックホストと、内部ポート探索の開始値です。`localhost` は `127.0.0.1` へ正規化し、IPv6 と外部公開可能なホストは拒否します。ブラウザーがこのポートへ直接接続することはありません。
...
Atlas のページとメタデータ取得先が応答可能になった後にだけ、**Open Atlas** リンクが表示されます。リンクは Local Data Studio と同じ接続元の `/atlas/{instance_id}/` を使うため、子プロセスの内部ポートはブラウザーへ公開されません。
...
選択された表示用の行と次元削減後の座標を含む Parquet ファイルは、`./cache/atlas/datasets` に保存されます。
...
`ATLAS_SAMPLE=N` は、SQL による絞り込み後、pandas の DataFrame を作成する前に DuckDB 内で決定的な抽出を行います。そのため、埋め込み計算、次元削減、キャッシュ Parquet のために DataFrame 化される行も最大 `N` 行です。`ATLAS_SAMPLE=0` の場合は、従来どおり選択行をすべて処理します。

テキストとプロンプトテンプレートは、現在の埋め込みバッチに必要な分だけ展開します。画像 bytes は一時的なディスク上の spool へ順次保存し、現在のバッチだけをメモリへ読み込みます。spool は成功、失敗、キャンセルのいずれでも削除します。ただし、full UMAP、t-SNE、PCA は抽出済みの埋め込み行列全体を必要とします。`anchor_transform` は、anchor 以外の埋め込みを transform バッチの処理後に保持しません。
...
準備中と実行中の Atlas プロセスは、合わせて `ATLAS_MAX_INSTANCES` 件まで保持します。実行中の instance は、Local Data Studio を再起動せずに停止できます。

```bash
curl -X DELETE http://127.0.0.1:8000/api/atlas/instances/INSTANCE_ID
```

instance ID は **Open Atlas** URL の `/atlas/` と末尾の `/` の間にある値です。この推測困難な値は中継先を識別するためのもので、認証機能ではありません。認証なしの Local Data Studio を信頼できないネットワークへ直接公開せず、ループバックへ bind するか SSH tunnel 経由で使用してください。
...
EDA レポート全体の容量は `EDA_CACHE_MAX_BYTES`、Embedding Atlas 関連キャッシュ全体の容量は `ATLAS_CACHE_MAX_BYTES` で制限されます。上限を超えると、それぞれ古いファイルから削除されます。JSON キャッシュは atomic に置き換えるため、書き込みが中断しても正常なキャッシュが途中までのファイルで上書きされません。
